if myHero.charName ~= "Rengar"  or not VIP_USER then return end
local version = 0.6
...
function initComponents()
 ...
 Menu.Passive:addParam("SelectPassive", "Choose your passive priority", SCRIPT_PARAM_LIST, 1, {"Q Priority", "W Priority", "E Priotity"})
 Menu.Passive:addParam("W", "W Priority when x% health", SCRIPT_PARAM_ONOFF, false)
 Menu.Passive:addParam("Low", " x% of health", SCRIPT_PARAM_SLICE, 15, 0, 100, 0) 
 --Menu.Passive.SelectPassive == 1
...
function PassivePriority()
  if target ~= nil and target.type == myHero.type and target.team ~= myHero.team and Passive then
     local targetH = target.maxHealth / 100 
      local targetH2 = targetH * Menu.Passiv.Low
      if Menu.Passive.W then
        if target.health <= targetH2 then
