...
local myth = {
	...
	skill = {
		q = {range = 625, del=0.500, speed=1900, w=70},
		w = {},
		e = {range = 400, del=0.3, speed=math.huge, w=160},
		r = {range = 1000, del = 0.3, speed = math.huge, w = 420}, -- player skill table 
	}
}
...
local function loadPred() -- load pred intergration
	UPL:AddSpell(_R, { speed = 550, delay = 0.0, range = 1000, width = 300, collision = false, aoe = true, type = "circular"})
end
...
function autoW()
	if settings.misc.autoW then
		for i, enemy in ipairs(GetEnemyHeroes()) do
			if enemy and AreaEnemyCount(enemy, settings.misc.range) >= settings.misc.autoSelf and enemy.visible then
				CastSpell(_W)
			end
		end
	end
...
function autoR()
	if settings.misc.autoR then
		for i, enemy in ipairs(GetEnemyHeroes()) do
			if enemy and ValidTarget(enemy, myth.skill.r.range) and AreaEnemyCount(enemy, 1000) >= settings.misc.autoUlt and enemy.visible then
				myth:cast("r", enemy)
			end
		end
	end
...
local function farm() -- minion farm
	...
	for i, m in pairs(myth.creep.objects) do
		if settings.lanec.q and damage("Q", m, me) >= m.health then
			myth:cast("q", m)
		end

		if settings.lanec.e and damage("E", m, me) >= m.health then
			myth:cast("e", m)
		end
	end
end
...
local function menu()
	...
	settings.harass:addParam("autoe", "Auto E in range", SCRIPT_PARAM_ONOFF, false)
	...
	settings:addSubMenu("Lane Clear", "lanec")
	settings.lanec:addParam("key", "Lane Clear Key", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("V"))
	settings.lanec:addParam("q", "Use Q", SCRIPT_PARAM_ONOFF, false)
	settings.lanec:addParam("e", "Use E", SCRIPT_PARAM_ONOFF, false)
	...
	settings.misc:addParam("autoW", "Auto W", SCRIPT_PARAM_ONOFF, false)
	settings.misc:addParam("autoSelf", "Enemy # before Auto W", SCRIPT_PARAM_SLICE, 2, 0, 5, 0)	
	settings.misc:addParam("range", "Enemy # in range before Auto R:", SCRIPT_PARAM_SLICE, 1500, 1000, 2000, 0)
...
function OnTick()
	...
	if settings.lanec.key then -- farm
		farm()
	end	
...
function OnDraw()
	...
	if myth.skill.r.range ~= nil and settings.draw.r and rready then
		DrawCircle(me.x, me.y, me.z, 1000, ARGB(125,0,150,255))
	end
end
