...
class 'Point' -- {
    ...
    function Point:closestPoint(spatialObject)
        if spatialObject:__type() == "Point" then
        ...
        elseif spatialObject:__type() == "LineSegment" then
            ...
            if t <= 0.0 then
                return spatialObject.points[1]
            elseif t >= 1.0 then
                return spatialObject.points[2]
            else
                return self:perpendicularFoot(Line(spatialObject.points[1], spatialObject.points[2]))
            end
        end
    end
-- }
...
class 'Path' -- {
    ...
    function Path:__type()
        return "Path"
    end

    function Path:__eq(spatialObject)
        return spatialObject:__type() == "Path" -- TODO
    end
    ...
    function Path:getLineSegments()
        if self.lineSegments == nil then
            self.lineSegments = {}
            for i = 1, #self.points - 1 do
                table.insert(self.lineSegments, LineSegment(self.points[i], self.points[i + 1]))
            end
        end
    ...
    function Path:draw(color, width)
        for i, lineSegment in ipairs(self:getLineSegments()) do
            lineSegment:draw(color, width)
        end
    end
