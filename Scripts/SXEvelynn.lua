if myHero.charName ~= "Evelynn"  or not VIP_USER then return end
local version = 0.1
...
function OnLoad()
   ...
   Spell_Q.collisionM['Evelynn'] = false
  Spell_Q.collisionH['Evelynn'] = false -- or false (sometimes, it's better to not consider it)
  Spell_Q.delay['Evelynn'] = .25
  Spell_Q.range['Evelynn'] = 500
  Spell_Q.speed['Evelynn'] = 1200
  Spell_Q.type['Evelynn'] = "DelayLine" -- (it has tail like comet)
  Spell_Q.width['Evelynn'] = 500
...
function CastR()
  if target ~= nil then
  RPos, RHitChance = HPred:GetPredict("R", target, myHero)
