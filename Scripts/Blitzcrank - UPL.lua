...
local myth = {
	name = "Blitzcrank - UPL", -- script name
	...
	skill = {
		q = {range = 900, del=0.250, speed=1800, w=70},
		w = {},
		e = {range = 125}, --Self
		r = {range = 600}, -- player skill table
	}
}
...
local function loadPred() -- load pred intergration
	UPL:AddSpell(_Q, { speed = 1800, delay = 0.250, range = 900, width = 70, collision = true, aoe = false, type = "linear"})
end
...
function autoR()
	if settings.misc.autoR then
		for i, enemy in ipairs(GetEnemyHeroes()) do
			if enemy and ValidTarget(enemy, myth.skill.r.range) and AreaEnemyCount(enemy, 900) >= settings.misc.autoUlt and enemy.visible then
				myth:cast("r", enemy)
			end
		end
	end
