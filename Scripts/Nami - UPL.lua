...
local myth = {
	...
	skill = {
		q = {range = 875, del=0.500, speed=1900, w=70},
		w = {range = 725},
		e = {range = 800, del=0.3, speed=math.huge, w=160},
		r = {range = 2750, del = 0.3, speed = math.huge, w = 420}, -- player skill table 
	}
}
...
local function loadPred() -- load pred intergration
	UPL:AddSpell(_Q, {speed = math.huge, delay = 0.8, range = 850, width = 0, collision = false, aoe = true, type = "circular"})
	UPL:AddSpell(_R, {speed = 859, delay = 0.25, range = 2750, width = 562, collision = false, aoe = true, type = "linear"})
end
...
function myth:cast(spell, targ) -- dynamic cast func
	...
	if spell == "wally" then
		...
		for _, ally in pairs(GetAllyHeroes()) do
			if ally ~= nil and not ally.dead and GetDistance(myHero, ally) <= myth.skill.w.range then
				if (ally.health / ally.maxHealth) * 100 <= settings.misc.hp and not ally.isMe then
					CastSpell(_W, ally)
				elseif (myHero.health/myHero.maxHealth)*100 <= settings.misc.hp then
					CastSpell(_W, myHero)
				end
			end
		end
...
function autoR()
	if settings.misc.autoR then
		for i, enemy in ipairs(GetEnemyHeroes()) do
			if enemy and ValidTarget(enemy, myth.skill.r.range) and AreaEnemyCount(enemy, myth.skill.r.range) >= settings.misc.autoUlt and enemy.visible then
				myth:cast("r", enemy)
			end
		end
	end
...
local function menu()
	...
	settings.harass:addParam("autow", "Auto W in range", SCRIPT_PARAM_ONOFF, false)
	...
	settings.misc:addParam("hp", "Health in Percent to AutoHeal", SCRIPT_PARAM_SLICE, 30, 0, 100, 0)
	settings.misc:addParam("mana", "Min. Mana to use AutoHeal", SCRIPT_PARAM_SLICE, 50, 0, 100, 0)
