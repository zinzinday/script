...
function checkUpdate()
	...
	ToUpdate.ScriptPath =  "/AMBER17/BoL/master/Blitzcrank.lua"
...
function Blitzcrank:OnDraw()
	if not myHero.dead then
		if self.Spells.Q.Ready() and self.Settings.drawing.qDraw then
			DrawCircle(myHero.x, myHero.y, myHero.z, self.Spells.Q.Range, ARGB(255,200,100,50))
		end
		if self.Spells.R.Ready() and self.Settings.drawing.rDraw then
			DrawCircle(myHero.x, myHero.y, myHero.z, self.Spells.R.Range, ARGB(255,200,100,50))
		end
		if ValidTarget(self.Target) then
			if self.Settings.drawing.targetText then
			...
			if self.Settings.drawing.targetCircle then
				DrawCircle(self.Target.x, self.Target.y, self.Target.z, 150, ARGB(255,125,125,125))
			end
			if self.Settings.drawing.line and self.Spells.Q.Ready() and GetDistance(self.Target) <= self.Spells.Q.Range + 100 then 
				local IsCollision = VP:CheckMinionCollision(self.Target, self.Target.pos, self.Spells.Q.Delay, self.Spells.Q.Width, self.Spells.Q.Range, self.Spells.Q.Speed, myHero.pos, nil, true)
				DrawLine3D(myHero.x, myHero.y, myHero.z, self.Target.x, self.Target.y, self.Target.z, 5, IsCollision and ARGB(125, 255, 0,0) or ARGB(125, 0, 255,0))
			end
		end
		UpdateWindow()
		if self.Settings.drawing.pourcentage then
			DrawText("Percentage Grab done : " .. tostring(math.ceil(self.pourcentage)) .. "%" ,18, WINDOW_H * 0.02,WINDOW_W * 0.2, 0xff00ff00)
		end
		if self.Settings.drawing.grabdone then
			DrawText("Grab Done : "..tostring(self.nbgrabwin),18, WINDOW_H * 0.02, WINDOW_W * 0.215, 0xff00ff00)
		end
		if self.Settings.drawing.grabfail then
			DrawText("Grab Miss : "..tostring(self.missedgrab),18, WINDOW_H * 0.02, WINDOW_W * 0.230, 0xFFFF0000)
		end
		if self.Settings.drawing.mana and self.manaShield ~= nil then
			DrawText("Passive's Shield : ".. tostring(math.ceil(self.manaShield)) .. "HP" ,18, WINDOW_H * 0.02, WINDOW_W * 0.245, 0xffffff00)
		end
	end
end

function Blitzcrank:OnWndMsg(Msg,Key)
	if Msg == WM_LBUTTONDOWN then
		...
		if self.Target and self.minD < 115 then
			if self.SelectedTarget and self.Target.charName == self.SelectedTarget.charName then
				...
				print("<font color=\"#FE2E64\"><b>Blitzcrank : Ass-Grabber - </b></font><font color=\"#FA58D0\">Target Lock: "..tostring(self.Target.charName).." </b></font>")
			end
		end
	end
end

function Blitzcrank:GetCustomTarget()
	if self.SelectedTarget ~= nil and ValidTarget(self.SelectedTarget, 2000) and GetDistance(self.SelectedTarget) < 1800 then
...
function Blitzcrank:myVariable()
	VP = VPrediction()
...
function Blitzcrank:OnProcessSpell(unit,spell)
	if unit and unit.isMe and spell and spell.name == "RocketGrab" then
		self.nbgrabtotal = self.nbgrabtotal + 1
...
function Blitzcrank:resetMenu()
	for i, enemy in pairs(self.myEnemyTable) do
		self.Settings.spell.qSpell[enemy.charName] = true
		self.Settings.autoGrab[enemy.charName] = true
	end
	self.Settings.autoGrab.autograbKey = false
end

function Blitzcrank:myMenu()
	self.Settings = scriptConfig("Blitzcrank: Ass-Grabber", "AMBER")
		...
		self.Settings:addSubMenu("["..myHero.charName.."] - AutoGrab Settings", "autoGrab")
			...
			self.Settings.autoGrab:addParam("orbWalk", "Move to Mouse Position", SCRIPT_PARAM_ONOFF, false)
		...
		self.Settings:addSubMenu("["..myHero.charName.."] - Spells Settings", "spell")
			self.Settings.spell:addSubMenu("["..myHero.charName.."] - (Q) Settings", "qSpell")
				...
				self.Settings.spell.qSpell:addParam("maxRange","Max (Q) Range for Grab", SCRIPT_PARAM_SLICE, 950, 400, 1000, 0)
				self.Settings.spell.qSpell:addParam("onlyE", "Only Grab if (E) Ready", SCRIPT_PARAM_ONOFF, false)
				for i, unit in pairs(self.myEnemyTable) do
		...
		self.Settings:addSubMenu("["..myHero.charName.."] - Draw Settings", "drawing")	
			...
			self.Settings.drawing:addParam("targetText", "Draw Current Target", SCRIPT_PARAM_ONOFF, true)
			self.Settings.drawing:addParam("targetCircle", "Draw Circle On Target", SCRIPT_PARAM_ONOFF, true)
			self.Settings.drawing:addParam("line", "Draw (Q) Line Helper", SCRIPT_PARAM_ONOFF, true)
			self.Settings.drawing:addParam("pourcentage", "Show Pourcentage", SCRIPT_PARAM_ONOFF, true)
			self.Settings.drawing:addParam("grabdone", "Show Grab Done", SCRIPT_PARAM_ONOFF, true)
			self.Settings.drawing:addParam("grabfail", "Show Grab Fail", SCRIPT_PARAM_ONOFF, true)
			self.Settings.drawing:addParam("mana", "Show Passive's shield", SCRIPT_PARAM_ONOFF, true)	
			
		self.Settings:addSubMenu("["..myHero.charName.."] - OrbWalk Settings (SBTW)", "Orbwalking")
			if SX then
...
function Blitzcrank:ManashieldStrength()
	return myHero.mana*0.5
end
...
function Blitzcrank:OnTick()
	...
	self:KillSteal()
	self.manaShield = tostring(math.ceil(self:ManashieldStrength()))
	if self.Settings.combo.comboKey then self:Combo(self.Target)
	elseif self.Settings.autoGrab.autograbKey then self:autoGrab(self.Target) end
end

function Blitzcrank:KillSteal()
	for i, enemy in pairs(self.myEnemyTable) do
		if ValidTarget(enemy) then
			local dmgR = getDmg("R", enemy, myHero)
			if GetDistance(enemy) <= self.Spells.R.Range - 50 and self.Spells.R.Ready() and enemy.health <= dmgR * 0.99 then
				CastSpell(_R)
			end
		end
	end
...
function Blitzcrank:autoGrab(unit)
	...
	if self.Settings.autoGrab.orbWalk then
...
function Blitzcrank:CastQ(unit)
	for i, enemy in pairs(self.myEnemyTable) do
		if self.Settings.spell.qSpell[unit.charName] then
			if GetDistance(unit) <= self.Settings.spell.qSpell.maxRange and GetDistance(unit) >= self.Settings.spell.qSpell.minRange and self.Spells.Q.Ready() then
				if self.Settings.spell.qSpell.onlyE then
					if self.Spells.E.Ready() then
						CastPosition,  HitChance,  Position = VP:GetLineCastPosition(unit, self.Spells.Q.Delay, self.Spells.Q.Width,self.Spells.Q.Range, self.Spells.Q.Speed, myHero, true)	
						if HitChance >= 2 then
							CastSpell(_Q, CastPosition.x, CastPosition.z)
						end
					end
...
function Blitzcrank:CastW(unit)
	if self.Settings.spell.wSpell.onlyQ then
		if self.Spells.Q.Ready() then
			...
			if GetDistance(unit) > 150 and GetDistance(unit) <= 400 and self.Spells.W.Ready() then
				CastSpell(_W)
			end
		end
	else
...
function Blitzcrank:CastE(unit)
	if (self.Spells.E.Ready() or os.clock() - self.lastE < 1) and GetDistance(unit) <= 250 then
		CastSpell(_E)
		myHero:Attack(unit)
		self.lastE = os.clock()
	end
...
function VPrediction:OnRecvPacket(p)
