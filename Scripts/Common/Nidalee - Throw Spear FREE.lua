...
local showClose = true -- Show trap locations that are close to you
...
---------------------------------------
 ...
 menu = scriptConfig("The Bestial Huntress", "Nidalee")
        menu:addParam("AutoQ", "Spear Harras", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("T"))
        menu:addParam("Combo", "Space To Combo", SCRIPT_PARAM_ONKEYDOWN, false, 32)
        menu:addParam("Heal", "Auto Heal", SCRIPT_PARAM_ONKEYTOGGLE, true, string.byte("J"))
        menu:addParam("SelfHeal", "Self Heal", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("K"))
        menu:addParam("FHeal", "Force Heal", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("L"))
		menu:addParam("HealAmount", "HealAmount", SCRIPT_PARAM_SLICE, 0.5,0.1,0.9,2)
		
		menu:addParam("ManaCheck", "Min Mana", SCRIPT_PARAM_SLICE, 0.3, 0, 1, 2)
		menu:addParam("WPos", "Lock on Traps", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("W"))
		menu:addParam("DrawCircleW", "DrawCircleW", SCRIPT_PARAM_ONOFF, true)
		menu:addParam("showLocationsInRange", "showLocationsInRange", SCRIPT_PARAM_SLICE, 800, 300, 1500, 1)
		menu:addParam("showClose", "showClose", SCRIPT_PARAM_ONOFF, true)
		menu:addParam("drawtrapSpots", "drawtrapSpots", SCRIPT_PARAM_ONOFF, true)
		menu:addParam("showCloseRange", "showCloseRange", SCRIPT_PARAM_SLICE, 800, 200, 1500, 1)
		menu:addParam("QHitChance", "Q Hit Chance", SCRIPT_PARAM_SLICE, 0.7, 0.1, 1, 2)
		
		menu:addParam("Debugging", "Debugging", SCRIPT_PARAM_ONOFF, false)
		
        menu:permaShow("AutoQ")
        menu:permaShow("Combo")
        menu:permaShow("FHeal")
        menu:permaShow("Heal")
				menu:permaShow("WPos")
...
function OnTick()
if recallStartTime ~= nil and GetTickCount() - recallStartTime > 8000 then
if menu.ManaCheck > (myHero.mana / myHero.maxMana) then
if menu.HealAmount > (myHero.health / myHero.maxHealth) then
        if menu.SelfHeal and HUMAN == true then SelfHeal() end
        if menu.Heal and HUMAN == true then Heal() end
        if menu.FHeal and menu.Heal and COUGAR == true then FHeal() end
end
end
end
wTraps()
...
Checks()
	...
	if menu.AutoQ and ts.target ~= nil then CastQ() end
	if menu.Combo and HUMAN == true then HCombo() end
	if menu.Combo and COUGAR == true then CCombo() end
	end
	
	function SelfHeal()
    if HUMAN == true then
        if myHero.health < (myHero.maxHealth*0.10) and myHero:CanUseSpell(_E) == READY then
            CastSpell(_E, myHero)
        end
    end
end
...
function Heal()
        if HUMAN == true then
                for i=1, heroManager.iCount do
                        ...
                        if allytarget.team == myHero.team and not allytarget.dead and GetDistance(allytarget) <= 600 and allytarget.health < (allytarget.maxHealth*0.10) and myHero:CanUseSpell(_E) == READY then
...
function HCombo()
    if HUMAN == true then
		if menu.UseQ and ts.target ~= nil then
			if predict ~= nil and GetDistance(predict) < qrange and not minionCollision(predict, QWidth, qrange) then
...
function OnDraw()
if HUMAN == true then
if menu.DrawCircleW then
DrawCircle(myHero.x, myHero.y, myHero.z, 890, 0x7F006E)
end
        for i,group in pairs(trapSpots) do
                if group.Enabled == true then
                        if menu.drawtrapSpots then
                                for x, trapSpot in pairs(group.Locations) do
                                        if GetDistance(trapSpot) < menu.showLocationsInRange then
                        ...
                        elseif menu.showClose then
                                for x, trapSpot in pairs(group.Locations) do
                                        if GetDistance(trapSpot) <= menu.showCloseRange then
                                                if GetDistance(trapSpot, mousePos) <= 300 then
                                                        trapColour = 0xFFFFFF
                                                else
                                                        trapColour = group.Colour
...
function minionCollision(predict, width, range)
	for _, minionObjectE in pairs(enemyMinions.objects) do
        if predict ~= nil and player:GetDistance(minionObjectE) < range then
            ex = player.x
            ez = player.z
            tx = predict.x
            tz = predict.z
