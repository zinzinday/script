...
function getDmg(spellname,target,owner,stagedmg,spelllvl)
	...
	elseif spellname == "Q" or spellname == "W" or spellname == "E" or spellname == "R" or spellname == "P" or spellname == "QM" or spellname == "WM" or spellname == "EM" then
		...
		elseif name == "Ahri" then
			if spellname == "Q" then DmgM = (25*Qlvl+15+.33*ap)*(stagedmg1+stagedmg3) DmgT = (25*Qlvl+15+.33*ap)*(stagedmg2+stagedmg3) -- stage1:Initial. stage2:way back. stage3:total.
		...
		elseif name == "Braum" then
			if spellname == "P" then DmgM = math.max((12*lvl+38)*(stagedmg1+stagedmg3),(2*lvl+12)*stagedmg2) --stage1-stage3:Stun. stage2:bonus damage.
		...
		elseif name == "Caitlyn" then
			...
			elseif spellname == "R" then DmgP = 225*Rlvl+25+2*ad
			end
		elseif name == "Cassiopeia" then
			if spellname == "Q" then DmgM = 40*Qlvl+35+.8*ap
			elseif spellname == "W" then DmgM = 10*Wlvl+15+.15*ap --xsec
			elseif spellname == "E" then DmgM = 35*Elvl+15+.55*ap
			elseif spellname == "R" then DmgM = 125*Rlvl+75+.6*ap
			end
		...
		elseif name == "Evelynn" then
			if spellname == "Q" then DmgM = 20*Qlvl+20+.45*ap+.5*bad
		...
		elseif name == "Ezreal" then
			if spellname == "Q" then DmgP = 20*Qlvl+15+.4*ap TypeDmg = 2 -- (bonus)
		...
		elseif name == "Gangplank" then
			...
			elseif spellname == "R" then DmgM = 45*Rlvl+30+.2*ap --xCannonball (25 cannonballs)
			end
		...
		elseif name == "Gragas" then
			if spellname == "P" then
			elseif spellname == "Q" then DmgM = math.max(40*Qlvl+40+.6*ap,(40*Qlvl+40+.6*ap)*1.5*stagedmg3) --Damage increase by up to 50% over 2 seconds. stage3: Max damage
		...
		elseif name == "Graves" then
			if spellname == "Q" then DmgP = math.max(35*Qlvl+25+.8*bad,(35*Qlvl+25+.8*bad)*1.7*stagedmg3) --xbullet , each bullet beyond the first will deal only 35% damage. stage3: Max damage
			elseif spellname == "W" then DmgM = 50*Wlvl+10+.6*ap
			elseif spellname == "R" then DmgP = math.max((100*Rlvl+150+1.5*bad)*(stagedmg1+stagedmg3),(80*Rlvl+120+1.2*bad)*stagedmg2) --stage1-stage3:Initial. stage2:Explosion.
			end
		...
		elseif name == "Kayle" then
			...
			elseif spellname == "E" then DmgM = 10*Elvl+10+.2*ap TypeDmg = 2 --xhit (bonus)
			end
		...
		elseif name == "Khazix" then
			...
			elseif spellname == "Q" then DmgP = math.max((25*Qlvl+30+1.2*bad)*stagedmg1,(25*Qlvl+30+1.2*bad)*1.3*stagedmg2,((25*Qlvl+30+1.2*bad)*1.3+10*lvl+1.04*bad)*stagedmg3) --stage1:Normal. stage2:to Isolated. stage3:Evolved to Isolated.
			elseif spellname == "W" then DmgP = 40*Wlvl+35+bad
		...
		elseif name == "KogMaw" then
			...
			elseif spellname == "R" then DmgM = 90*Rlvl+90+.3*ap+.5*bad
			end
		...
		elseif name == "Lissandra" then
			...
			elseif spellname == "W" then DmgM = 40*Wlvl+30+.6*ap
		...
		elseif name == "Lux" then
			if spellname == "P" then DmgM = 10+10*lvl
		...
		elseif name == "Maokai" then
			...
			elseif spellname == "W" then DmgM = (1.5*Wlvl+7.5+.04*ap)*tmhp/100
			...
			elseif spellname == "R" then DmgM = 50*Rlvl+50+.5*ap+(50*Rlvl+150)*stagedmg3 -- +2 per point of damage absorbed (max 200/250/300). stage3: Max damage
			end
		...
		elseif name == "Morgana" then
			...
			elseif spellname == "R" then DmgM = math.max(75*Rlvl+100+.7*ap,(75*Rlvl+100+.7*ap)*2*stagedmg3) --x2 If the target stay in range for the full duration. stage3: Max damage
			end
		...
		elseif name == "Nidalee" then
			...
			elseif spellname == "QM" then DmgM = (math.max(4,30*Rlvl-40,40*Rlvl-70)+ad+.24*ap)*(1+1.5*(tmhp-thp)/tmhp) --Deals 33% increased damage against Hunted
		...
		elseif name == "Pantheon" then
			if spellname == "Q" then DmgP = (40*Qlvl+25+1.4*bad)+(40*Qlvl+25+1.4*bad)*0.5*math.floor((tmhp-thp)/85)
		...
		elseif name == "Sion" then
			if spellname == "Q" then DmgM = 57.5*Qlvl+12.5+.9*ap
			elseif spellname == "W" then DmgM = 50*Wlvl+50+.9*ap
			end
		...
		elseif name == "Sona" then
			if spellname == "P" then DmgM = math.max((math.max(7*lvl+6,8*lvl+3,9*lvl-2,10*lvl-8,15*lvl-78)+.2*ap)*2*stagedmg1,(math.max(7*lvl+6,8*lvl+3,9*lvl-2,10*lvl-8,15*lvl-78)+.2*ap)*(stagedmg2+stagedmg3)) TypeDmg = 2 --stage1: Staccato , stage2:Diminuendo or Tempo
			elseif spellname == "Q" then DmgM = 50*Qlvl+.5*ap
			elseif spellname == "R" then DmgM = 100*Rlvl+50+.5*ap
			end
		elseif name == "Soraka" then
			if spellname == "Q" then DmgM = 25*Qlvl+15+.4*ap
			elseif spellname == "E" then DmgM = 30*Elvl+10+.4*ap+.05*mmana
			end
		...
		elseif name == "Tristana" then
			...
			elseif spellname == "E" then DmgM = math.max((40*Elvl+70+ap)*(stagedmg1+stagedmg3),(25*Elvl+25+.25*ap)*stagedmg2) --stage1-3:Active. stage2:Passive.
		...
		elseif name == "Viktor" then
			if spellname == "Q" then DmgM = 45*Qlvl+35+.65*ap
			elseif spellname == "E" then DmgM = math.max(45*Elvl+25+.7*ap,(45*Elvl+25+.7*ap)*1.3*stagedmg3) --Augment Death deal 30% additional damage. stage3: Max damage
			elseif spellname == "R" then DmgM = math.max((100*Rlvl+50+.55*ap)*stagedmg1,(20*Rlvl+20+.25*ap)*stagedmg2,(100*Rlvl+50+.55*ap+(20*Rlvl+20+.25*ap)*7)*stagedmg3) --stage1:initial. stage2: xsec (7 sec). stage3: Max damage
			end
