...
local version = 1.064
local champions = {["Riven"] = true, ["Xerath"] = true, ["Orianna"] = true, ["Draven"] = true, ["Lissandra"] = true, ["Irelia"] = true, ["Heimerdinger"] = true, ["Kennen"] = true}
...
function OnLoad()
    ...
    if OrbwalkManager == nil then print("Check your SimpleLib file, isn't working... The script can't load without SimpleLib. Try to copy-paste the entire SimpleLib.lua on your common folder.") end
...
function _Kennen:LoadVariables()
    ...
    self.QSpell = _Spell({Slot = _Q, DamageName = "Q", Range = 950, Width = 50, Delay = 0.125, Speed = 1700, Collision = true, Aoe = false, Type = SPELL_TYPE.LINEAR}):AddDraw()
...
function _Kennen:Combo()
    ...
    if self.RSpell:IsReady() and self.Menu.Combo.useR <= #self.RSpell:ObjectsInArea(GetEnemyHeroes()) and Config.Combo.useR > 0 then CastSpell(self.RSpell.Slot) end
...
function _Kennen:EnemyIsMarked(enemy)
    return self.EnemiesMarked[enemy.charName] == true
end
...
function _Irelia:__init()
    ...
    self.R  = { Slot = _R, DamageName = "Q", Range = 1000, Width = 120, Delay = 0, Speed = 1600, LastCastTime = 0, Collision = false, Aoe = true, Type = SPELL_TYPE.LINEAR, IsReady = function() return myHero:CanUseSpell(_R) == READY end, Mana = function() return myHero:GetSpellData(_R).mana end, Damage = function(target) return getDmg("R", target, myHero) end}
    self:LoadVariables()
    self:LoadMenu()
end
...
function _Irelia:Combo()
    ...
    if ValidTarget(target, self.TS.range) then
        ...
        if self.Menu.Combo.useW then self:CastW(target) end
        if self.Menu.Combo.useR and myHero.health/myHero.maxHealth < 0.9 then self:CastR(target) end
    end
end
...
function _Irelia:Clear()
    if 100 * myHero.mana / myHero.maxMana >= self.Menu.Clear.Mana  then
        ...
        if ValidTarget(minion) and GetDistanceSqr(myHero, minion) < self.Menu.Clear.maxQRange * self.Menu.Clear.maxQRange then
            self:CastQ(minion)
        end
...
function _Irelia:LastHit()
    if 100 * myHero.mana / myHero.maxMana >= self.Menu.LastHit.Mana then
        if self.Menu.LastHit.useQ then
            local minion = self.QSpell:LastHit({ Mode = LASTHIT_MODE.ALWAYS, UseCast = false})
            if ValidTarget(minion) and GetDistanceSqr(myHero, minion) < self.Menu.LastHit.maxQRange * self.Menu.LastHit.maxQRange then
                self:CastQ(minion)
            end
        end
        if self.Menu.LastHit.useE then 
...
function _Lissandra:LoadVariables()
    ...
    self.QSpell = _Spell(self.Q):AddDraw():SetRangeFunction(function() return self.Q.Range end)
    self.WSpell = _Spell(self.W):AddDraw()
...
function _Lissandra:LoadMenu()
    ...
    AddCreateObjCallback(
        function(obj)
            ...
            elseif obj.name:lower():find("linemissile") and GetDistanceSqr(myHero, obj) < 100 * 100 and os.clock() - self.E.LastCastTime < 0.5 then
    ...
    AddDeleteObjCallback(
        function(obj)
            ...
            elseif self.E.MissileObj ~= nil and obj.name:lower():find("linemissile") and GetDistanceSqr(self.E.MissileObj, obj) < 100 * 100 and GetDistanceSqr(obj, self.E.MissileObj) < 80 * 80 then 
...
function _Orianna:__init()
    ...
    self.Q  = { Slot = _Q, DamageName = "Q", Range = 825, Width = 130, Delay = 0, Speed = 1200, Type = SPELL_TYPE.CIRCULAR, LastCastTime = 0, Collision = false, Aoe = true, IsReady = function() return myHero:CanUseSpell(_Q) == READY end, Mana = function() return myHero:GetSpellData(_Q).mana end, Damage = function(target) return getDmg("Q", target, myHero) end}
    self.W  = { Slot = _W, DamageName = "W", Range = 255, Width = 255, Delay = 0, Speed = 1200, Type = SPELL_TYPE.SELF, LastCastTime = 0, Collision = false, Aoe = true, IsReady = function() return myHero:CanUseSpell(_W) == READY end, Mana = function() return myHero:GetSpellData(_W).mana end, Damage = function(target) return getDmg("W", target, myHero) end}
...
function _Orianna:LoadVariables()
    ...
    self.QSpell = _Spell(self.Q):AddDraw()
    self.WSpell = _Spell(self.W):AddDraw():SetSourceFunction(function() return self.Position end)
    self.ESpell = _Spell(self.E):AddDraw()
    self.RSpell = _Spell(self.R):AddDraw():SetSourceFunction(function() return self.Position end)
    ...
    AddCreateObjCallback(
        function(obj)
            ...
            elseif obj.name:lower():find("orianna") and obj.name:lower():find("ball") and obj.name:lower():find("flash") then
                self.Position = myHero
            elseif obj.name:lower():find("linemissile") and GetDistanceSqr(self.Position, obj) < 100 * 100 and (os.clock() - self.Q.LastCastTime < 0.5 or os.clock() - self.E.LastCastTime < 0.5) then
                self.Position = obj
            end
        end
    )
        ...
        AddCastSpellCallback(
            function(iSpell, startPos, endPos, targetUnit) 
                if iSpell == self.R.Slot and #self.RSpell:ObjectsInArea(GetEnemyHeroes()) == 0  then
                    self.R.Sent = os.clock() - OrbwalkManager:Latency()
                end
            end
        )
    end
...
function _Xerath:LoadVariables()
    ...
    self.WSpell = _Spell(self.W):AddDraw()
    self.ESpell = _Spell(self.E):AddDraw()
    self.RSpell = _Spell(self.R):AddDraw():SetRangeFunction(function() return 2000 + 1200 * myHero:GetSpellData(self.R.Slot).level end)
end
...
function _Xerath:LoadMenu()
    ...
    AddCreateObjCallback(
        function(obj)
            if obj and os.clock() - self.E.LastCastTime < 0.5 and obj.name:lower():find("linemissile") and GetDistanceSqr(myHero, obj) < 100 * 100 then 
                self.E.Missile = obj
        ...
        AddCastSpellCallback(
            function(iSpell, startPos, endPos, targetUnit) 
                if iSpell == self.R.Slot then
                    self.R.Sent = true
                    DelayAction(function() self.R.Sent = false end, self.TimeLimit + OrbwalkManager:Latency())
                end 
            end
        )
    end
...
function _Xerath:CastQ(target)
    if self.Q.IsReady() and ValidTarget(target) then
        if not self.Q.IsCharging then
            ...
            local Position = Prediction:GetPredictedPos(target, {Delay = delay})
            if Position~=nil and GetDistanceSqr(myHero, Position) < self.Q.MaxRange * self.Q.MaxRange then
...
function _Riven:CastQ(targ)
    ...
    if self.Q.IsReady() and ValidTarget(target) then
        local CastPosition, WillHit = self.QSpell:GetPrediction(target)
        if ValidTarget(target, self.Q.Range) then
            if OrbwalkManager:InRange(target) and self.Q.State == 0 and OrbwalkManager:CanAttack() and self:RequiresAA(target) then return end
            if self:ShouldUseAA() then return end
            if CastPosition~=nil then
                CastSpell(self.Q.Slot, CastPosition.x, CastPosition.z)
                return
...
function _Riven:Run()
    ...
    if self.E.IsReady() and os.clock() - self.Q.LastCastTime > 0.25 + OrbwalkManager:Latency() then
        CastSpell(self.E.Slot, mousePos.x, mousePos.z)
        self.E.LastCastTime = os.clock()
        return
    end
    if self.Q.IsReady() and os.clock() - self.E.LastCastTime > 0.25 + OrbwalkManager:Latency() then
        CastSpell(self.Q.Slot, mousePos.x, mousePos.z)
        self.Q.LastCastTime = os.clock()
        return
    end
...
function _Draven:LoadMenu()
    ...
    AddCreateObjCallback(
        function(obj)
            if obj and os.clock() - self.R.LastCastTime < 0.5 and obj.name:lower():find("linemissile") and GetDistanceSqr(myHero, obj) < 100 * 100 then
                self.R.Obj = obj
            end
        end
    )
...
function _Draven:LastHit()
    ...
    for i, minion in pairs(self.EnemyMinions.objects) do
       if ValidTarget(minion, self.AA.Range(minion)) and self.Menu.LastHit.useQ and self.Q.IsReady() then
            local time = OrbwalkManager:WindUpTime() - OrbwalkManager:ExtraWindUp() + OrbwalkManager:Latency() + GetDistance(myHero.pos, minion.pos) / self.ProjectileSpeed - 100/1000
        ...
        if self.Menu~=nil and self.Menu.Catch and (( self.Menu.CatchMode == 1 and not OrbwalkManager:IsNone()) or self.Menu.CatchMode == 2 ) then
            ...
            local i = 1
                ...
                if axe ~= nil and axe.valid and os.clock() - time <= self.LimitTime and self:InRadius(axe) and not self:InTurret(axe) then
                    ...
                    local time = timeLeft - ((GetDistance(myHero, AxeCatchPositionFromHero)) / myHero.ms) - (OrbwalkManager:WindUpTime() + OrbwalkManager:Latency() + champ.AA.Range(champ.TS.target) / self.ProjectileSpeed)
                    --Is in AxeRange
                    if self:InAxeRadius(axe) then
                        CanAttack = true
                    --Can Attack Meanwhile
                    elseif self.Menu.AABetween and time >= 0 and OrbwalkManager:CanAttack(0) then --+ orbwalker:WindUpTime() -
                        CanAttack = true
                        if champ.Menu.Misc.developer then print("Can Attack Meanwhile") end
                    elseif OrbwalkManager:AnimationTime() * 1 + (GetDistance(myHero, AxeCatchPositionFromHero)) / myHero.ms < timeLeft then --+ orbwalker:WindUpTime()
                        CanAttack = true
                    ...
                    if CanMove then
                        ...
                        OrbwalkManager:DisableMovement()
                        if not self:InAxeRadius(axe) and not OrbwalkManager:IsAttacking() then
...
function _Queue:OnProcessSpell(unit, spell)
    if unit and spell and spell.name and unit.isMe then
        ...
        if spell.name:lower():find("attack") then
            ...
            ,spell.windUpTime - OrbwalkManager:Latency() * 2)
        elseif spell.name:lower():find("riventricleave") then
            self:Remove("Q", 1)
        elseif spell.name:lower():find("rivenmartyr") then
            self:Remove("W", 1)
