...
function OnLoad()
	...
	initComponents()
 --[[ HPred:AddSpell(, {type = "DelayLine", delay = .25, range = 1175, speed = 1200, collisionM = true, collisionH = true, width = 160})
 -- HPred:AddSpell("Q", "Lux", {type = "DelayLine", delay = 0.25, range = 1175, speed = math.huge, width = 180})
   HPred:AddSpell("R", "Lux", {type = "PromptLine", delay = 1.75, range = 3340, speed = 3000, width = 380})
    HPred:AddSpell("E", "Lux", {type = "PromptCircle", delay = 0.25, range = 1100, speed = 1300 , radius = 350})
...
local E2Pos, E2Hitchance = HPred:GetPredict(Lux_E2, target, pos where you casted E)
local RPos, RHitchance = HPred:GetPredict(Lux_R, target, myHero)
 ...
 -- print(percentage)
  --print(pewpew)
  --print(GeRDamage())
 -- print(myHero:GetSpellData(_R).level)
...
HPred = HPrediction()
 ...
 -- Menu.selectProdSettings == 1 or 2
	Menu.ProdSettings:addParam("SelectProdiction", "Select Prodiction", SCRIPT_PARAM_LIST, 1, {"HProdiction", "VPrediction", "Devine Prod"})
...
function ProdictionCombo()
  ...
  if target ~= nil and target.type == myHero.type and target.type == myHero.type and target.team ~= myHero.team then
...
function DevineQ()
 -- print("Boss")
  ...
  if Menu.ProdSettings.SelectProdiction == 3 then
			...
			local LuxQ = LineSS(skills.SkillQ.speed, skills.SkillQ.range, skills.SkillQ.width, skills.SkillQ.delay, 0)
			local State, Position, perc = DP:predict(target, LuxQ)
...
function DevineE()
  ...
  if Menu.ProdSettings.SelectProdiction == 3 then
			local target = DPTarget(target)
			local LuxE = CircleSS(skills.SkillE.speed, skills.SkillE.range, skills.SkillE.width, skills.SkillE.delay, 0)
			local State, Position, perc = DP:predict(target, LuxE)
			if State == SkillShot.STATUS.SUCCESS_HIT then 
...
function DevineR()
  ...
  if Menu.ProdSettings.SelectProdiction == 3 then
			local target = DPTarget(target)
			local LuxR = LineSS(skills.SkillR.speed, skills.SkillR.range, skills.SkillR.width, skills.SkillR.delay, 0)
			local State, Position, perc = DP:predict(target, LuxR)
			if State == SkillShot.STATUS.SUCCESS_HIT then 
...
function KSNoPassive()
 -- print(rDmg)
  if not Passive then
    if target ~= nil and target.type == myHero.type and target.team ~= myHero.team  and target.visible and ValidTarget(target) and not target.dead then
    --  if paardebek ~= nil then
        ...
        --  print(rDmg)
            if Menu.ProdSettings.SelectProdiction == 2 and ValidTarget(target)then
             ...
             -- print("KSQ")
              ...
              if target.health <= eDmg and Eready and Menu.Ads.KS.useE then
              EVP()
            --  print("KSE")
              end
        ...
        --    print("big combo")
          ...
          elseif Menu.ProdSettings.SelectProdiction == 3 and ValidTarget(target) then
              if target.health <= qDmg and Qready and Menu.Ads.KS.useQ then
              DevineQ()
             -- print("KSQ")
              end
          ...
          if Stun and target.health <= eDmg + rDmg + pDmg and Rready and Eready and ValidTarget(target, skills.SkillE.range) and Menu.Ads.KS.useE and Menu.Ads.KS.useR then
            DevineE()
            DevineR()
            --print("big combo2")
            KSAA()
  ...
  if Passive then
      if target ~= nil and target.type == myHero.type and target.team ~= myHero.team and target.visible and target.valid and not target.dead then
        ...
        local target = heroManager:GetHero(i)
         ...
         -- print(ipad + rDmg)
          if Menu.ProdSettings.SelectProdiction == 2 then 
          --  print(" check van engels")
          --[[if paardebek ~= nil then
            if target.health < paardebek then
            KSAA()
          end--]]
          end
            if target.health <= rDmg + pDmg and Rready and Menu.Ads.KS.useR and ValidTarget(target, skills.SkillR.range) then
            RVP()
            print("VP")
           --print("KANKERKANKERKANKERKANKERKANKER")
            end
            if Stun and target.health <= eDmg + rDmg + pDmg and Qready and Eready and Rready and ValidTarget(target, skills.SkillE.range) and Menu.Ads.KS.useE and Menu.Ads.KS.useR then
            EVP()
            RVP()
           -- print("1566")
            KSAA()
            end
          elseif Menu.ProdSettings.SelectProdiction == 1  and ValidTarget(target) then
            if target.health <= paardebek then
            KSAA()
            end
            if target.health <= rDmg + pDmg and Rready and Menu.Ads.KS.useR and ValidTarget(target, skills.SkillR.range)then
            ...
            if Stun and target.health <= rDmg + pDmg + eDmg  and Rready and Eready and Menu.Ads.KS.useE and Menu.Ads.KS.useR then
          ...
          -- print(" veel te hoog")
            ...
            if Stun and target.health <= rDmg + pDmg + eDmg + pDmg and Rready and Eready and Menu.Ads.KS.useE and Menu.Ads.KS.useR then
